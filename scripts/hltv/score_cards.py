...
def predict_model_log_views(meta: dict, team: str, opponent: str,
                            weights: dict | None,
                            ranking: dict | None,
                            fixtures: list | None = None) -> float:
    """LIM-pro model: same features_from_row path as fit_pov_weights."""
    from shorts.fit_pov_weights import (features_from_row, predict_log_views,
                                        row_from_card)
    from shorts.pro_context import derby_heat

    weights = weights if weights is not None else load_model_weights()
    if not weights:
        return 0.0
    heat = derby_heat(team, opponent, fixtures or [])
    row = row_from_card(meta, team, opponent, ranking, derby_views=heat)
    return round(predict_log_views(features_from_row(row), weights), 3)
...
def score_card(
    ...
    *,
    ...
    model_views = predict_model_log_views(
        meta, team, opponent, None, ranking, fixtures)
    return {
