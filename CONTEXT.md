...
- **Render Batch**: Consecutive demo rounds rendered in a single csdm invocation. Default size is **3** (`--batches N`). On failure mid-batch, the whole batch is re-rendered on resume.
...
- **Batches flag**: `--batches N` on `render_pov.py` and `pipeline.py` (default 3). Pipeline forwards the flag to render step 6.
