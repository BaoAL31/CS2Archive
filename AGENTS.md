...
- **YouTube scheduling** — default `--publish-at auto`: next future **10:00 or 16:30 Australia/Sydney** slot on a free day via the **YouTube API** (channel uploads playlist). Local ledger (`youtube/.publish_schedule.json`) deprecated. Explicit `--publish-at "YYYY-MM-DD HH:MM"` still schedules exactly.
