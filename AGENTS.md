...
- **Overlay (step 4) runs by default:** utility throw flight PiP clips (~1–2 min/throw via CSDM/HLAE; 20+ throws ≈ 30–60 min), one PiP per unique non-straightforward lineup (repeat throws from the same release cell show once; straightforward tosses are skipped). Lineup freeze frames in the main POV are OFF by default — opt in with `--freeze` (pipeline + `overlay_pov.py`). Keyboard input overlay is OFF by default — opt in with `--keyboard` (pipeline + `overlay_pov.py`). Skip the whole step with `--until 3` or `--raw-only`. `--overlay-only` is a deprecated no-op.
