...
**Daily notable selector (cron):** `scripts/faceit/scrape_notable.py` reports notable matches (multi-pro + single-pro standout; `--hours 48`/`--limit 5` defaults, `--today-only`, `--exclude-today`, `--json`, reusable `collect()`) and `scripts/faceit/daily_notable.py` is the scheduled daily job — scrapes, ranks, picks **3** matches/day, falls back to a persistent pool (`.data/notable_daily.json`) from previous days when today can't fill 3. Idempotent per day (`--force` redo). `--download` also fetches each picked demo + builds backlog cards; `--install-cron [--at 09:00]` / `--remove-cron` manages the Windows scheduled task `CS2ArchiveFaceitDaily` (daily 09:00, uses the `cs2archive` conda python).
