...
- **Overlay (step 4) runs by default** (dual-upload on): keyboard states via demoparser2 + utility throw flight PiP clips (~1–2 min/throw via CSDM/HLAE; 20+ throws ≈ 30–60 min). Skip with `--until 3` or `--raw-only`; `--overlay-only` renders only the overlay variant.
...
- **Dual-upload (default):** two variants per backlog entry — raw `youtube/{run_id}/` and overlay `youtube/{run_id}_overlay/` (title suffix `| Input Overlay + Utility Cam`, badge + note in thumbnail/description). Each has its own `upload_meta.json`; re-running re-does only missing work. **FACEIT POVs default to overlay-only** (no raw variant produced) — the keyboard+util-cam version IS the product for pugs; `--raw-only` still forces just the raw variant.
...
- **VP9 trick:** render 2560×1440 even for 1080p-targeted uploads — YouTube gives 1440p+ VP9 (higher bitrate), sharper at 1080p too. All scripts default to 2560×1440, **h264_nvenc CQ 15** end-to-end.
...
- **HLTV demo acquisition** — CloakBrowser, persistent profile `.cloak-hltv-profile/`. Undersized archives (<1MB) are not cache hits. Fallback: `--demo` with local `.rar`/`.dem`.
