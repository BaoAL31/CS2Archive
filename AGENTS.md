...
- **Dual-upload (default):** two variants per backlog entry — raw `youtube/{run_id}/` and overlay `youtube/{run_id}_overlay/` (title suffix `| Input Overlay + Utility Cam`, badge + note in thumbnail/description). Each has its own `upload_meta.json`; re-running re-does only missing work.
