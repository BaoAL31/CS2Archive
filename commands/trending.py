...
Usage:
    python main.py trending [--url-only] [--count N] [--hours N]
"""
...
def handle(args: argparse.Namespace) -> None:
    asyncio.run(_cmd(args.url_only, args.count, args.hours))


async def _cmd(url_only: bool, count: int, hours: int) -> None:
    from scrapers.trending import find_trending

    results = await find_trending(count=count, hours=hours)
    if not results:
        return
    ...
    table = Table(title=f"Trending CS2 Matches (last {hours}h)", show_lines=True)
