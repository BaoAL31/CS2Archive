...
2. **Individual POV** — `python scripts/faceit/create_faceit_backlog.py <demo_path> --player <nick> --map <map>` creates a single card for one player (`--priority high|mid|low`, default `high`), then the standard `pipeline.py` runs the full flow. Same output shape (minus rating/ELO fields), same `backlog/faceit/{priority}/` layout.
